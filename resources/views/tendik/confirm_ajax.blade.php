...
@else
<form action="{{ url('/tendik/' . $tendik->id_tendik . '/delete_ajax') }}" method="POST" id="form-delete-tendik">
    @csrf
    @method('DELETE')
    <div id="modal-master" class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Hapus Data Tendik</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.status) {
                        ...
                        tableTendik.ajax.reload(); // Adjust to your datatable's variable name
                    } else {
                        $('.error-text').text('');
